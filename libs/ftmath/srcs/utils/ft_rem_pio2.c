...
/*   Updated: 2019/08/23 10:10:33 by abaurens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int32_t	medium_c(double x, double *y, int32_t ix, int32_t hx)
{
	double		fn;
	double		w;
	double		r;
	...
	fn = ((double)(((double_t)x * invpio2) + 0x1.8p52) - 0x1.8p52);
	r = x - fn * pio2_1;
	w = fn * pio2_1t;
	{
		j = ix >> 20;
		...
		if (i > 16)
		{
			y[0] = medium_calc(r, fn, pio2_2, pio2_2t);
			i = j - ((get_high_word(y[0]) >> 20) & 0x7ff);
			if (i > 49)
				y[0] = medium_calc(r, fn, pio2_3, pio2_3t);
		}
	}
	y[1] = (r - y[0]) - w;
	return (((int)fn));
...
static int	ret2(int32_t hx, int factor, double x, double *y)
{
	...
	{
		z = x - factor * pio2_1;
		y[0] = z - factor * pio2_1t;
		y[1] = (z - y[0]) - factor * pio2_1t;
		return (factor);
	}
	z = x + factor * pio2_1;
	y[0] = z + factor * pio2_1t;
