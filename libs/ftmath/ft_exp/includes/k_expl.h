...
/*
 * See s_ft_expl.c for more comments about __k_ft_expl().
 *
...
/*
 * Kernel for ft_expl(x).  x must be finite and not tiny or huge.
 * "tiny" is anything that would make us underflow (|A6*x^6| < ~LDBL_MIN).
 * "huge" is anything that would make fn*L1 inexact (|x| > ~2**17*ln2).
 */
static inline void
__k_ft_expl(long double x, long double *hip, long double *lop, int *kp)
{
	...
	*kp = n >> LOG2_INTERVALS;
	...
	/* Evaluate ft_expl(endpoint[n2] + r1 + r2) = tbl[n2] * ft_expl(r1 + r2). */
	z = r * r;
#if 0
...
static inline void
k_hft_expl(long double x, long double *hip, long double *lop)
{
	float twopkm1;
	int k;

	__k_ft_expl(x, hip, lop, &k);
...
hft_expl(long double x)
{
	long double hi, lo, twopkm2;
	int k;

	twopkm2 = 1;
	__k_ft_expl(x, &hi, &lo, &k);
...
__ldexp_cft_expl(long double complex z, int expt)
{
