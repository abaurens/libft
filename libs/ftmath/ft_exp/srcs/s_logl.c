...
/*   Updated: 2019/08/22 14:23:37 by abaurens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long double	ft_logl(long double x)
{
	...
	EXTRACT_LDBL80_WORDS(exp, mant, x);
	k = -16383;
	if (exp == 0 || exp >= 0x8000) {
		...
		EXTRACT_LDBL80_WORDS(exp, mant, x);
		k = -16383 - 65;
	} else if (exp >= 0x7fff || (mant & 0x8000000000000000ULL) == 0)
		RETURN1(rp, x + x);
	k += exp;
	ix = mant & 0x7fffffffffffffffULL;
	dk = k;
...
long double	log1pl(long double x)
{
	...
	if (exp < 0x3fff) {
		ax = exp & 0x7fff;
		if (ax >= 0x3fff) {
			if (ax == 0x3fff && mant == 0x8000000000000000ULL)
				RETURNP(-1 / zero);
			RETURNP((x - x) / (x - x));
		}
		if (ax <= 0x3fbe) {
	...
	SET_LDBL_EXPSIGN(x, 0x3fff);
	twopminusk = 1;
	SET_LDBL_EXPSIGN(twopminusk, 0x7ffe - (exp & 0x7fff));
	f_lo *= twopminusk;
